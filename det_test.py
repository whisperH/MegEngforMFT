...
def make_parser():
    ...
    parser.add_argument(
        "-f", "--file",
        default="detector/configs/retinanet_res50_coco_3x_800size.py",
        type=str, help="net description file"
    )
    parser.add_argument(
        "-w", "--weight_file",
        default='detector/log/log-of-retinanet_res50_coco_3x_800size/epoch_10.pkl',
        type=str, help="weights file",
    )
    parser.add_argument(
    ...
    parser.add_argument("-se", "--start_epoch", default=9, type=int)
    parser.add_argument("-ee", "--end_epoch", default=10, type=int)
    return parser
...
def main():
    ...
    for epoch_num in range(args.start_epoch, args.end_epoch + 1):
        if args.weight_file:
            ...
            weight_file = "detector/log/log-of-{}/epoch_{}.pkl".format(
        ...
        json_path = "detector/log/log-of-{}/epoch_{}.json".format(
            os.path.basename(args.file).split(".")[0], epoch_num
        )
...
def build_dataloader(dataset_dir, cfg):
    val_dataset = data_mapper[cfg.test_dataset["name"]](
        os.path.join(dataset_dir, cfg.test_dataset["name"]),

...
def make_parser():
    ...
    parser.add_argument(
        "-n", "--devices", default=4, type=int, help="total number of gpus for training",
    )
    parser.add_argument(
...
def main():
    ...
    if args.devices > 1:
        ...
        print("cost time is {} h ".format((end-start)/3600))
...
def worker(args):
    ...
    for epoch in range(model.cfg.max_epoch):
        ...
        if dist.get_rank() == 0:
            save_path = "detector/log/log-of-{}/epoch_{}.pkl".format(
                os.path.basename(args["det_model_file"]).split(".")[0], epoch
            )
            mge.save(
...
def build_dataset(dataset_dir, cfg):
    ...
    data_cfg["root"] = os.path.join(dataset_dir, data_name)
